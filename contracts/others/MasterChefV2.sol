...
contract MasterChefV2 is Ownable, ReentrancyGuard {
  ...
  function add(
    ...
    if (_isRegular) {
      totalRegularAllocPoint += _allocPoint;
    } else {
      totalSpecialAllocPoint += _allocPoint;
    }
  ...
  function pendingCake(uint256 _pid, address _user) external view returns (uint256) {
    ...
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      ...
      uint256 cakeReward = (multiplier * cakePerBlock(pool.isRegular) * pool.allocPoint) /
        (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint);
      accCakePerShare += (cakeReward * ACC_CAKE_PRECISION) / lpSupply;
    }
  ...
  function cakePerBlock(bool _isRegular) public view returns (uint256 amount) {
    if (_isRegular) {
      amount = (MASTERCHEF_CAKE_PER_BLOCK * cakeRateToRegularFarm) / CAKE_RATE_TOTAL_PRECISION;
    } else {
      amount = (MASTERCHEF_CAKE_PER_BLOCK * cakeRateToSpecialFarm) / CAKE_RATE_TOTAL_PRECISION;
    }
  }
  ...
  function cakePerBlockToBurn() public view returns (uint256 amount) {
    amount = (MASTERCHEF_CAKE_PER_BLOCK * cakeRateToBurn) / CAKE_RATE_TOTAL_PRECISION;
  }
  ...
  function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
    ...
    if (block.number > pool.lastRewardBlock) {
      ...
      if (lpSupply > 0 && totalAllocPoint > 0) {
        ...
        uint256 cakeReward = (multiplier * cakePerBlock(pool.isRegular) * pool.allocPoint) /
          totalAllocPoint;
        pool.accCakePerShare += (cakeReward * ACC_CAKE_PRECISION) / lpSupply;
      }
  ...
  function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
    ...
    user.rewardDebt =
      (((user.amount * multiplier) / BOOST_PRECISION) * pool.accCakePerShare) /
      ACC_CAKE_PRECISION;
    poolInfo[_pid] = pool;
  ...
  function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
    ...
    user.rewardDebt =
      (((user.amount * multiplier) / BOOST_PRECISION) * pool.accCakePerShare) /
      ACC_CAKE_PRECISION;
    poolInfo[_pid].totalBoostedShare -= (_amount * multiplier) / BOOST_PRECISION;
  ...
  function burnCake(bool _withUpdate) public onlyOwner {
    ...
    uint256 pendingCakeToBurn = multiplier * cakePerBlockToBurn();
  ...
  function updateCakeRate(
    ...
    require(
      _burnRate + _regularFarmRate + _specialFarmRate == CAKE_RATE_TOTAL_PRECISION,
  ...
  function updateBoostMultiplier(
    ...
    user.rewardDebt =
      (((user.amount * _newMultiplier) / BOOST_PRECISION) * pool.accCakePerShare) /
      ACC_CAKE_PRECISION;
